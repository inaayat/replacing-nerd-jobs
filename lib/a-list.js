...
async function getUserProfile(userId, { requirePublic = true } = {}) {
  const rows = await db()`
    SELECT u.id, m.username, m.public_profile
    FROM users u
  ...
  return {
    userId: rows[0].id,
    displayName: publicDisplayName(rows[0]) || 'You',
  };
}
...
export async function getUserPublicProfile(userId, { requirePublic = true } = {}) {
  ...
  return {
    userId: user.id,
    displayName: publicDisplayName(membership) || 'Your log',
...
export async function getLeaderboard() {
  ...
  const [users, memberships, watchRows] = await Promise.all([
    db()`
      ...
      WHERE m.public_profile = true AND m.username IS NOT NULL
  ...
  return users.map((user) => {
    ...
    return {
      userId: user.id,
      displayName: publicDisplayName(membership),
      totalSeen: summary.totalSeen,
      totalSavings: summary.totalSavings,
      totalCharged: summary.totalCharged,
      totalBilled: summary.totalBilled,

...
export async function respondToShowingInvite(userId, { inviteId, action }) {
  ...
  if (watchId && match) {
    const patch = missingWatchFieldsFromInvite(match, invite);
    if (patch) {
      const nextLocation = patch.location ?? match.location;
      const nextTicket = patch.ticket_cents !== undefined ? patch.ticket_cents : match.ticket_cents;
      const nextFormat = patch.format ?? match.format ?? '';
      const nextTmdb = patch.tmdb_id !== undefined ? patch.tmdb_id : match.tmdb_id;
      await db()`
        UPDATE alist_watches SET
          location = ${nextLocation},
          ticket_cents = ${nextTicket},

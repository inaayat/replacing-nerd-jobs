...
export { watchlistItemMatchesLogged } from '../amc-a-lister/engine/watchlist-match.js';
...
async function findMatchingWatchForUser(userId, snapshot) {
  const rows = await db()`
    ...
    WHERE user_id = ${userId}
      ...
      AND in_theaters IS NOT FALSE
  `;
  // Prefer an exact theater match when one exists, otherwise any same-movie
  // same-date row — accepting must never invent a duplicate outing.
  const exact = rows.find((row) => watchesMatchForTogether(row, snapshot));
  if (exact) return exact;
  return rows.find((row) => watchesMatchSameMovieNight(row, snapshot)) || null;
}

/** Backfill blank theater/ticket/format/tmdb on an existing watch from a richer source. */
async function fillMissingWatchDetails(userId, existing, source) {
  const patch = missingWatchFieldsFromInvite(existing, source);
  if (!patch) return null;
  const nextLocation = patch.location ?? existing.location;
  const nextTicket = patch.ticket_cents !== undefined ? patch.ticket_cents : existing.ticket_cents;
  const nextFormat = patch.format ?? existing.format ?? '';
  const nextTmdb = patch.tmdb_id !== undefined ? patch.tmdb_id : existing.tmdb_id;
  await db()`
    UPDATE alist_watches SET
      location = ${nextLocation},
      ticket_cents = ${nextTicket},
      format = ${nextFormat},
      tmdb_id = ${nextTmdb},
      saw_alone = false,
      updated_at = now()
...
export async function inviteToShowing(fromUserId, { watchId, username }) {
  ...
  if (match) {
    const filled_fields = await fillMissingWatchDetails(target.user_id, match, source);
...
export async function listShowingInvites(userId) {
  await ensureSchema();
  await purgeOrphanShowingInvites();
  const rows = await db()`
    SELECT
...
/**
 * Drop invites whose sender or recipient no longer has a Neon Auth account.
 * App `users` rows can linger after Auth deletion; CASCADE only runs when the
 * app user row is deleted via /api/me.
 */
export async function purgeOrphanShowingInvites() {
  try {
    await db()`
      DELETE FROM alist_watch_invites i
      WHERE NOT EXISTS (
          SELECT 1 FROM neon_auth."user" au WHERE au.id::text = i.from_user_id
        )
        OR NOT EXISTS (
          SELECT 1 FROM neon_auth."user" au WHERE au.id::text = i.to_user_id
        )
    `;
  } catch (err) {
    const msg = String(err?.message || err);
    if (/neon_auth/i.test(msg) || /does not exist/i.test(msg)) return;
    throw err;
  }
}

/** Sender cancels a pending outgoing invite. */
export async function cancelShowingInvite(userId, inviteId) {
  await ensureSchema();
  ...
  const rows = await db()`
    ...
    WHERE id = ${id} AND from_user_id = ${userId}
    LIMIT 1
  `;
  const invite = rows[0];
  ...
  await db()`
    ...
    WHERE id = ${invite.id} AND from_user_id = ${userId}
  `;
  return { cancelled: true, id: invite.id };
}
...
export async function respondToShowingInvite(userId, { inviteId, action }) {
  ...
  // Accepting: if they already logged this movie on this date, tag that row
  // instead of creating a duplicate.
  const match = await findMatchingWatchForUser(userId, invite);
  let createdWatch = null;
  let watchId = match?.id || null;
  let filled_fields = null;

  if (watchId && match) {
    filled_fields = await fillMissingWatchDetails(userId, match, invite);
  } else if (!watchId) {
...
export function watchlistFromRow(row) {
  ...
  return {
    ...
    notes: row.notes,
    watch_at_home_override: row.watch_at_home_override === true,
    created_at: row.created_at,
    updated_at: row.updated_at,
  };
}
...
/**
 * Drop matching want-to-watch rows after a title is added to the watch log.
 * Matches by TMDB id and/or exact title (case-insensitive).
 */
export async function clearWatchlistForLoggedMovie(userId, { tmdbId = null, title = null } = {}) {
  await ensureSchema();
  const tid = tmdbId != null && Number.isFinite(Number(tmdbId)) ? Number(tmdbId) : null;
  const normalizedTitle = title != null ? String(title).trim() : '';
  if (tid == null && !normalizedTitle) return [];

  return db()`
    DELETE FROM alist_watchlist
    WHERE user_id = ${userId}
      AND (
        (${tid}::int IS NOT NULL AND tmdb_id = ${tid})
        OR (
          ${normalizedTitle} <> ''
          AND lower(trim(title)) = lower(${normalizedTitle})
        )
      )
    RETURNING id, title, tmdb_id
  `;
}
...
export async function listTvWatches(userId) {
  ...
  return db()`
    SELECT w.id, w.watched_on::text AS watched_on, w.title, w.tmdb_id,
           ...
           w.created_at, w.updated_at, c.poster_path, c.year

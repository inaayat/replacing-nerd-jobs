...
export function publicWatchFromRow(row) {
  return {
    id: row.id,
    watched_on: row.watched_on,
    title: row.title,
...
export async function getUserPublicProfile(userId) {
  ...
  const summary = computeSummary(summaryWatches, membership);
  ...
  return {
    ...
    watches: watchRows.map(publicWatchFromRow),
  };
}
...
export async function findMovieWatchlistDuplicate(userId, { tmdb_id, title }) {
  await ensureSchema();
  const tmdbId = parseTmdbId(tmdb_id);
  const titleKey = normalizeTitleKey(title);
  if (!tmdbId && !titleKey) return null;
  ...
  const rows = await db()`
    ...
    WHERE w.user_id = ${userId}
      AND (
        (${tmdbId}::int IS NOT NULL AND w.tmdb_id = ${tmdbId})
        OR (${titleKey} != '' AND lower(trim(w.title)) = ${titleKey})
      )
    ORDER BY w.created_at DESC
    LIMIT 1
  `;
...
export async function findTvWatchlistDuplicate(userId, { tmdb_id, title }) {
  await ensureSchema();
  const tmdbId = parseTmdbId(tmdb_id);
  const titleKey = normalizeTitleKey(title);
  if (!tmdbId && !titleKey) return null;
...
export async function removeMovieWatchlistMatches(userId, { tmdb_id, title }) {
  await ensureSchema();
  const tmdbId = parseTmdbId(tmdb_id);
  const titleKey = normalizeTitleKey(title);
  let removed = 0;
  ...
  if (titleKey) {
    const rows = await db()`
      DELETE FROM alist_watchlist
      WHERE user_id = ${userId} AND lower(trim(title)) = ${titleKey}
      RETURNING id
    `;
    removed += rows.length;
  }
...
export async function removeTvWatchlistMatches(userId, { tmdb_id, title }) {
  await ensureSchema();
  const tmdbId = parseTmdbId(tmdb_id);
  const titleKey = normalizeTitleKey(title);
  let removed = 0;

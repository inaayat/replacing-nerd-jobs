...
export function membershipPriceTiers(membership) {
  ...
  if (membership?.rate_setup_complete === false) return [];
...
export function monthlyRateForMonth(chargeMonthStr, membership) {
  ...
  if (!tiers.length) return Number(membership?.current_cents) || 0;
...
export function nextChargeMonth(chargeMonthStr) {
  const year = Number(chargeMonthStr.slice(0, 4));
  const month = Number(chargeMonthStr.slice(5, 7));
...
/** Fold legacy first-month promo into price_tiers, then bill only from tiers. */
export function foldPromoIntoTiers(membership, firstChargeMonth) {
  if (membership?.promo_folded) {
    return { tiers: membershipPriceTiers(membership), changed: false };
  }

  const promo = Number(membership?.promo_cents);
  const tiers = membershipPriceTiers(membership);

  if (!Number.isFinite(promo) || promo <= 0 || !tiers.length) {
    return { tiers, changed: true };
  }
  ...
  const rateAtFirst = monthlyRateForMonth(firstChargeMonth, { price_tiers: tiers });
  if (promo === rateAtFirst) {
    return { tiers, changed: true };
  }

  const second = nextChargeMonth(firstChargeMonth);
  const rest = [{ effective_on: second, cents: rateAtFirst }];
  const seen = new Set([second]);

  for (const tier of tiers) {
    const effective_on = tier.effective_on <= firstChargeMonth ? second : tier.effective_on;
    if (seen.has(effective_on)) continue;
    seen.add(effective_on);
    rest.push({ effective_on, cents: tier.cents });
  }

  return {
    tiers: normalizePriceTiers([
      { effective_on: firstChargeMonth, cents: promo },
      ...rest,
    ]) || tiers,
    changed: true,
  };
...
export function monthlyBillForMonth(chargeMonthStr, membership, _sortedChargeMonths) {
...
export function isExcludedTheaterLocation(location) {
...
export function theaterStats(watches) {
  ...
  return [...map.values()]
    .map((row) => ({
      ...
      avgRating: row.ratedCount
        ? Math.round((row.ratingSum / row.ratedCount) * 10) / 10
        : null,
    }))
    .sort((a, b) => b.count - a.count);
}

export function topTheatersByRating(theaters, { minRated = 2, limit = 10 } = {}) {
  return theaters
    .filter((theater) => theater.ratedCount >= minRated && theater.avgRating != null)
    .sort((a, b) => b.avgRating - a.avgRating || b.count - a.count || a.location.localeCompare(b.location))
    .slice(0, limit);
}
...
/** Whole-star bucket for rating profile charts (4.5 → 4). */
export function ratingStarBucket(rating) {
...
export function ratingDistribution(watches) {
  ...
  for (const w of rated) {
    buckets[ratingStarBucket(w.rating)] += 1;
  }
...
export function actorStats(watches, castByTmdbId) {
  ...
  for (const watch of watches) {
    ...
    if (watch.watched_on > movie.watched_on) {
      movie.watched_on = watch.watched_on;
    }
    if (!watch.dnf && watch.rating != null) {
      movie.ratings.push(watch.rating);
    }
  }
  ...
  for (const movie of movies.values()) {
    ...
    for (const actor of cast) {
      ...
      if (!name) continue;
      if (!map.has(name)) {
        map.set(name, { actor: name, count: 0, ratingSum: 0, ratedCount: 0, films: [] });
      }
      const row = map.get(name);
      row.count += 1;
      row.films.push({ title: movie.title, watched_on: movie.watched_on });
      if (movieRating != null) {
  ...
  return [...map.values()]
    .map((row) => ({
      actor: row.actor,
      count: row.count,
      avgRating: row.ratedCount ? Math.round((row.ratingSum / row.ratedCount) * 10) / 10 : null,
      ratedCount: row.ratedCount,
      films: row.films.sort((a, b) => b.watched_on.localeCompare(a.watched_on)),
    }))
    .sort((a, b) => b.count - a.count || (b.avgRating || 0) - (a.avgRating || 0));
}

export function topActorsByRating(actors, { minRated = 2, limit = 10 } = {}) {
  return actors
    .filter((actor) => actor.ratedCount >= minRated && actor.avgRating != null)

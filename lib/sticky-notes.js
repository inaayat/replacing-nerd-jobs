...
export async function getState(userId) {
  await ensureSchema();
  const sql = db();
  const [notes, collections, arrows, ink, legend, wikis] = await Promise.all([
    sql`
      SELECT id, text, rich, media, tags, color_key, icon_key, status, collection_id,
    ...
    `,
    sql`SELECT kind, key, label, image_url FROM sn_legend WHERE user_id = ${userId}`,
    sql`
      SELECT collection_id, doc, updated_at
      FROM sn_wiki WHERE user_id = ${userId}
    `,
  ]);

  const legendOut = { colors: {}, icons: {}, customIcons: {} };
  for (const row of legend) {
    if (row.kind === 'color') legendOut.colors[row.key] = row.label;
    if (row.kind === 'icon') legendOut.icons[row.key] = row.label;
    if (row.kind === 'custom-icon') {
      const key = normalizeCustomIconKey(row.key);
      const icon = normalizeCustomIcon({ label: row.label, imageUrl: row.image_url });
      if (key && icon) legendOut.customIcons[key] = icon;
    }
  }
...
async function applyOne(sql, userId, op) {
  ...
  switch (op.op) {
    case 'note.upsert': {
      ...
      const media = note.media ? JSON.stringify(note.media) : null;
      const tags = note.tags?.length ? JSON.stringify(note.tags) : null;
      await sql`
        INSERT INTO sn_notes (
          id, user_id, text, rich, media, tags, color_key, icon_key, status, collection_id,
          x, y, w, h, pinned, source_url, source_title,
          created_at, updated_at, filed_at
        ) VALUES (
          ${note.id}, ${userId}, ${note.text}, ${rich}::jsonb, ${media}::jsonb, ${tags}::jsonb,
    ...
    case 'note.move':
      ...
      return;
    case 'note.resize': {
      const { w, h } = resizeNoteSize(Number(op.w) || NOTE_W_MIN, Number(op.h) || NOTE_H_MIN);
      await sql`
    ...
    case 'note.categorize': {
      ...
      if ('iconKey' in op) {
        const builtin = ICON_KEYS.includes(op.iconKey) ? op.iconKey : null;
        const custom = normalizeCustomIconKey(op.iconKey);
        if (custom) {
          await sql`
            UPDATE sn_notes SET
              icon_key = (
                SELECT key FROM sn_legend
                WHERE user_id = ${userId} AND kind = 'custom-icon' AND key = ${custom}
              ),
              updated_at = ${ts}
            WHERE id = ANY(${list}) AND user_id = ${userId}
          `;
          ...
          await sql`
            UPDATE sn_notes SET icon_key = ${builtin}, updated_at = ${ts}
            WHERE id = ANY(${list}) AND user_id = ${userId}
          `;
        }
      }
    ...
    case 'collection.create': {
      ...
      if (!name) return;
      const status = op.status === 'memory' ? 'memory' : 'board';
      const filedAt = status === 'memory' ? ts : null;
      await sql`
        INSERT INTO sn_collections (id, user_id, name, status, created_at, updated_at, filed_at)
        VALUES (${String(op.id)}, ${userId}, ${name}, ${status}, ${ts}, ${ts}, ${filedAt})
        ON CONFLICT (id) DO NOTHING
      `;
      return;
    }
    ...
    case 'wipe':
      ...
      return;
    case 'wiki.set': {
      const cid = String(op.collectionId || '');
      if (!cid) return;
      const doc = JSON.stringify(normalizeDoc(op.doc));
      await sql`
        INSERT INTO sn_wiki (collection_id, user_id, doc, created_at, updated_at)
        SELECT ${cid}, ${userId}, ${doc}::jsonb, ${ts}, ${ts}
        WHERE EXISTS (SELECT 1 FROM sn_collections WHERE id = ${cid} AND user_id = ${userId})
        ON CONFLICT (collection_id) DO UPDATE SET
          doc = EXCLUDED.doc,
          updated_at = EXCLUDED.updated_at
        WHERE sn_wiki.user_id = ${userId}
          AND sn_wiki.updated_at <= EXCLUDED.updated_at
      `;
      return;
    }
    case 'legend.set': {
      const kind = op.kind === 'color'
        ...
        : op.kind === 'icon'
          ...
          : op.kind === 'custom-icon'
            ...
            : null;
      if (!kind) return;
      const key = kind === 'custom-icon' ? normalizeCustomIconKey(op.key) : String(op.key || '');
      if (kind === 'color' && !COLOR_KEYS.includes(key)) return;
      if (kind === 'icon' && !ICON_KEYS.includes(key)) return;
      if (kind === 'custom-icon' && !key) return;
      const label = String(op.label ?? '').trim();
      const custom = kind === 'custom-icon'
        ? normalizeCustomIcon({ label, imageUrl: op.imageUrl })
        : null;
      if (!label || (kind === 'custom-icon' && !custom)) {
        await sql`
          DELETE FROM sn_legend WHERE user_id = ${userId} AND kind = ${kind} AND key = ${key}
        `;
        if (kind === 'custom-icon') {
          await sql`
            UPDATE sn_notes SET icon_key = NULL, updated_at = ${ts}
            WHERE user_id = ${userId} AND icon_key = ${key}
          `;
        }
      } else {
        await sql`
          INSERT INTO sn_legend (user_id, kind, key, label, image_url)
          VALUES (${userId}, ${kind}, ${key}, ${label}, ${custom?.imageUrl || null})

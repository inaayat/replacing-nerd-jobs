...
export async function getState(userId) {
  await ensureSchema();
  const sql = db();
  const [notes, collections, arrows, legend] = await Promise.all([
    sql`
      SELECT id, text, color_key, icon_key, status, collection_id,
    ...
    case 'note.upsert': {
      ...
      await sql`
        INSERT INTO sn_notes (
          id, user_id, text, color_key, icon_key, status, collection_id,
          x, y, w, h, pinned, source_url, source_title,
          created_at, updated_at, filed_at
        ) VALUES (
          ${note.id}, ${userId}, ${note.text}, ${note.colorKey}, ${note.iconKey},
          ${note.status},

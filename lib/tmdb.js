...
// TMDB hands back every credit it has, so an actor's "filmography" arrives
// padded with talking-head documentaries, awards specials, and archive-footage
// cameos. Left in, they crowd out the films someone actually means: Tom Hanks
// has 181 credits, and the padding outnumbers his post-2010 features.
const DOCUMENTARY_GENRE_ID = 99;
const NON_PERFORMANCE_CHARACTER = /^\s*(self|himself|herself|themselves)\b|archive footage|uncredited/i;
const NOTABLE_VOTE_FLOOR = 25;

/**
 * Whether a raw TMDB credit represents a film worth counting.
 *
 * Documentary and "as themselves" screening only applies to acting credits —
 * a documentary filmmaker's *directing* credits are the whole point of their
 * filmography, so screening those out would empty the query.
 */
export function isNotableCredit(credit, { acting = false } = {}) {
  if (!credit?.id) return false;
  if ((credit.vote_count ?? 0) < NOTABLE_VOTE_FLOOR) return false;
  if (!acting) return true;
  if ((credit.genre_ids || []).includes(DOCUMENTARY_GENRE_ID)) return false;
  return !NON_PERFORMANCE_CHARACTER.test(credit.character || '');
}

/**
 * Rank and screen a raw `/person/{id}/movie_credits` payload.
 *
 * Ordering matters more than it looks: callers cap this list to a scan depth,
 * and TMDB returns credits in roughly chronological order, so slicing the raw
 * response keeps someone's *oldest* films and discards their best-known ones.
 * Ranking by vote count first means the cap keeps the films people mean.
 * `popularity` is deliberately only a tie-break — it is heavily recency-biased.
 *
 * Kept pure and exported so the selection can be tested without a TMDB key.
 */
export function selectPersonCredits(credits, role, quality = 'notable') {
  const empty = { ids: [], totalRaw: 0, totalEligible: 0, order: 'vote_count.desc', relaxed: false };
  if (!credits) return empty;

  const pool = [];
  if (role === 'person-acted' || role === 'person-any') {
    for (const credit of credits.cast || []) {
      if (credit?.id) pool.push({ credit, acting: true });
    }
  }
  if (role === 'person-directed' || role === 'person-any') {
    const crew = role === 'person-directed'
      ? (credits.crew || []).filter((c) => c.job === 'Director')
      : (credits.crew || []);
    for (const credit of crew) {
      if (credit?.id) pool.push({ credit, acting: false });
    }
  }

  // One film, one entry — a person can hold several credits on the same title.
  const byFilm = new Map();
  for (const entry of pool) {
    const existing = byFilm.get(entry.credit.id);
    // Prefer the crew record when both exist: it isn't subject to the
    // "as themselves" screen, so directing your own documentary still counts.
    if (!existing || (existing.acting && !entry.acting)) byFilm.set(entry.credit.id, entry);
  }
  const unique = [...byFilm.values()];

  let eligible = quality === 'everything'
    ? unique
    : unique.filter((entry) => isNotableCredit(entry.credit, { acting: entry.acting }));

  // Screening everything out is worse than showing a noisy answer, so an
  // obscure filmography falls back to the unscreened list and says so.
  const relaxed = !eligible.length && unique.length > 0;
  if (relaxed) eligible = unique;

  eligible.sort((a, b) => (
    (b.credit.vote_count ?? 0) - (a.credit.vote_count ?? 0)
    || (b.credit.popularity ?? 0) - (a.credit.popularity ?? 0)
    // Deterministic tail so the same query hashes to the same cache key.
    || a.credit.id - b.credit.id
  ));

  return {
    ids: eligible.map((entry) => entry.credit.id),
...
/** A person's films, most-voted first. See `selectPersonCredits`. */
export async function getPersonFilmIds(personId, role, { apiKey, quality = 'notable' } = {}) {
  const credits = await tmdbFetch(`/person/${personId}/movie_credits`, {}, {
    apiKey,
    allowNotFound: true,
  });
  return selectPersonCredits(credits, role, quality);
}

export async function discoverMovieIds(params = {}, { apiKey, pages = 2 } = {}) {
  const ids = [];
  let totalResults = 0;
  const sort = params.sort_by || 'popularity.desc';
  ...
  for (let page = 1; page <= pages; page += 1) {
    ...
    if (page === 1) totalResults = Number(data?.total_results) || 0;
  ...
  // `totalResults` is how many films match, not how many we read — discover is
  // always a ranked sample of an open-ended set and provenance must say so.
  return { ids: [...new Set(ids)], totalResults, order: sort };
}
...
/** How well a name answers what was typed: exact beats prefix beats substring. */
export function nameMatchScore(name, needle) {
...
/**
 * People matching a query, most recognisable first.
 *
 * Ordering is the whole feature here. This used to slice to `limit` *before*
 * sorting and then order alphabetically within a directors-first partition,
 * which is why "greta" listed Cervantes, Chi and Garbo above Greta Gerwig and
 * "anderson" put Brad Anderson above Wes. Rank on how well the name matches,
 * then on TMDB popularity, and only cut the list afterwards.
 */
  ...
  return (data.results || [])
    .map((p) => ({
      ...
      known_for: (p.known_for || [])
        ...
        .map((k) => k.title)
        .filter(Boolean),
      _score: nameMatchScore(p.name, query),
      _popularity: Number(p.popularity) || 0,
    }))
    .sort((a, b) => (
      b._score - a._score
    ...
    .slice(0, limit)
    .map(({ _score, _popularity, ...person }) => person);
}

    ...
    _schemaReady = (async () => {
      ...
      await sql`
        CREATE TABLE IF NOT EXISTS alist_watchlist (
          ...
          tmdb_id          INT,
          notes            TEXT,
          watch_at_home_override BOOLEAN NOT NULL DEFAULT false,
          created_at       TIMESTAMPTZ NOT NULL DEFAULT now(),
          updated_at       TIMESTAMPTZ NOT NULL DEFAULT now()
        )
      `;
      await sql`
        ALTER TABLE alist_watchlist
        ADD COLUMN IF NOT EXISTS watch_at_home_override BOOLEAN NOT NULL DEFAULT false
      `;
      await sql`
      ...
      `;
      await sql`
        CREATE TABLE IF NOT EXISTS alist_tv_ranks (
          user_id      TEXT NOT NULL REFERENCES users(id) ON DELETE CASCADE,
          tmdb_id      INT  NOT NULL,
          position     INT  NOT NULL,
          title        TEXT NOT NULL,
      ...
      `;
      await sql`
        CREATE INDEX IF NOT EXISTS alist_tv_ranks_user_position
          ON alist_tv_ranks (user_id, position)
      `;
      // Cubes are private to their owner. `is_public` / `github_pr_url` /
      // `published_at` are vestigial columns from the retired publish-to-
      // catalog flow: nothing reads or writes them. They stay because dropping
      // a column is a breaking change (run that in the Neon console if ever).
      await sql`
      ...
      `;
      await sql`DROP INDEX IF EXISTS pc_cubes_public`;
      await sql`
      ...
      `;
      // Formatted body (bold / bullets / numbers) as blocks; `text` stays the
      // plain projection that memory searches. Null means a plain-text note
      // written before formatting existed.
      await sql`
        ALTER TABLE sn_notes
        ADD COLUMN IF NOT EXISTS rich JSONB
      `;
      // Optional compact image / social-video attachment. Embed URLs are never
      // stored; clients derive allowlisted official embeds from `media.url`.
      await sql`
        ALTER TABLE sn_notes
      ...
      `;
      // Board ink: text written on the board itself. No status column, because
      // it can never be filed — a wipe deletes these rows.
      await sql`
        CREATE TABLE IF NOT EXISTS sn_ink (
          id          TEXT PRIMARY KEY,
          user_id     TEXT NOT NULL REFERENCES users(id) ON DELETE CASCADE,
          text        TEXT NOT NULL,
          x           REAL NOT NULL DEFAULT 24,
      ...
      `;
      await sql`
        CREATE INDEX IF NOT EXISTS sn_ink_user ON sn_ink (user_id)
      `;
      await sql`
      ...
      `;
      await sql`
        ALTER TABLE sn_legend
        ADD COLUMN IF NOT EXISTS image_url TEXT
      `;
      // One wiki page per collection. Opening always works; the row is created
      // lazily on first edit. Cascade with the collection.
      await sql`
        CREATE TABLE IF NOT EXISTS sn_wiki (
          collection_id TEXT PRIMARY KEY REFERENCES sn_collections(id) ON DELETE CASCADE,
          user_id       TEXT NOT NULL REFERENCES users(id) ON DELETE CASCADE,
          doc           JSONB NOT NULL,
          created_at    TIMESTAMPTZ NOT NULL DEFAULT now(),
          updated_at    TIMESTAMPTZ NOT NULL DEFAULT now()
      ...
      `;
      await sql`
        CREATE INDEX IF NOT EXISTS sn_wiki_user ON sn_wiki (user_id)
      `;
      await sql`
        CREATE TABLE IF NOT EXISTS wedding_boards (
          user_id     TEXT PRIMARY KEY REFERENCES users(id) ON DELETE CASCADE,
          payload     JSONB NOT NULL,
          updated_at  TIMESTAMPTZ NOT NULL DEFAULT now()
        )

...
export async function getSuitcaseState(userId) {
  ...
  const rows = await db()`
    ...
    LIMIT 1
  `;
  if (!rows[0]) return { activeSuitcaseId: null, suitcases: [], prefs: { betaViews: false } };
  ...
  return {
    ...
    prefs: { betaViews: !!prefs.betaViews },
...
export async function putSuitcaseState(userId, state) {
  ...
  const prefs = { betaViews: !!(state.prefs && state.prefs.betaViews) };
  await db()`

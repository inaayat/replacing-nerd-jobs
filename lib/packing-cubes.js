...
export function validateCube(cube) {
  ...
  const itemsError = validateItemList(cube.items);
  ...
  if (cube.addOns != null) {
    ...
    for (const addOn of cube.addOns) {
      ...
      if (!Array.isArray(addOn.items) || addOn.items.length < 1) {
        return `Add-on "${addOn.title}" needs at least 1 item.`;
      }
      if (addOn.items.length > MAX_ITEMS) {
        return `Add-ons are capped at ${MAX_ITEMS} items.`;
      }
      const addOnItemsError = validateItemList(addOn.items);
...
export async function listOwnCubes(userId) {
  await ensureSchema();
  const rows = await db()`
    SELECT id, user_id, title, blurb, tags, items, add_ons, created_at, updated_at
    FROM pc_cubes
...
export async function insertCube(userId, cube) {
  await ensureSchema();
  const rows = await db()`
    INSERT INTO pc_cubes (id, user_id, title, blurb, tags, items, add_ons)
    VALUES (
      ${cube.id}, ${userId}, ${cube.title}, ${cube.blurb},
      ${JSON.stringify(cube.tags)}, ${JSON.stringify(cube.items)},
      ${JSON.stringify(cube.addOns || [])}
    )
    RETURNING id, user_id, title, blurb, tags, items, add_ons, created_at, updated_at
  `;

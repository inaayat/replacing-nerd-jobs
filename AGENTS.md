...
- `lib/plot-points-query.js` is imported by BOTH `api/plot-points.js` and the browser (`plot-points/app.js`), so the builder controls and the server engine share one field catalog. Keep it dependency-free ESM — no `node:` imports, no npm packages — or the browser build breaks.
...
- Tests (all pure functions, no deps/secrets; there is no `npm test` wrapper):
  ...
  - `node scripts/test-plot-points-query.mjs` — Plot Points query engine (`lib/plot-points-query.js`)

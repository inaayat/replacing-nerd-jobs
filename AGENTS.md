...
- Vercel's Hobby plan allows at most 12 serverless functions per deployment; this repo currently uses 9. Prefer adding a `?route=` branch to an existing handler plus a rewrite in `vercel.json` before creating a new `api/*.js` file. (This is why all Plot Points endpoints live in `api/plot-points.js`.)

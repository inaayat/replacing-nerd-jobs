...
- For local UI work without Vercel or secrets, serve the repo root as static files, e.g. `python3 -m http.server 8080`. All client-side pages work this way — notably the JSON-driven `sporcle-spinoff/` trivia platform (catalog + player) at `http://127.0.0.1:8080/sporcle-spinoff/`. The `api/*` routes and `/private` gate do NOT run under a plain static server.
- Tests: `node scripts/test-billing.mjs` runs the A-List billing unit tests (pure functions in `lib/a-list-billing.js`, no deps/secrets). There is no `npm test` wrapper.

...
.page-header h1 {{ font-size: 1.25rem; font-weight: 500; margin: 0 0 4px; letter-spacing: 0.5px; }}
.total-banner {{
  font-size: 0.7rem; color: var(--mid); letter-spacing: 1.2px; text-transform: uppercase;
}}
.total-banner strong {{ color: var(--ink); font-size: 1.05rem; letter-spacing: 0; text-transform: none; font-weight: 500; }}
...
.detail-prog-name {{ font-size: 0.9rem; font-weight: 500; }}
.detail-breadcrumb {{
  font-size: 0.65rem; opacity: 0.7; flex: 1;
  white-space: nowrap; overflow: hidden; text-overflow: ellipsis;
}}
.detail-close {{
  background: none; border: none; color: var(--bg);
  font-size: 1rem; cursor: pointer; opacity: 0.7; margin-left: auto;
  padding: 0 0 0 12px; flex-shrink: 0;
}}
.detail-close:hover {{ opacity: 1; }}
.detail-body {{ padding: 16px; display: grid; grid-template-columns: 1fr 1fr; gap: 16px; }}
.detail-section {{ }}
...
.detail-meta-item {{
  border: 1px solid var(--faint); padding: 8px 10px;
}}
.detail-meta-val {{ font-size: 0.88rem; font-weight: 500; color: var(--ink); }}
.detail-meta-key {{ font-size: 0.6rem; color: var(--mid); text-transform: uppercase; letter-spacing: 1px; margin-top: 2px; }}
.detail-desc {{
  font-size: 0.73rem; line-height: 1.6; color: var(--ink);
}}
.detail-desc.empty {{ color: var(--mid); font-style: italic; }}
.detail-bsl {{
  font-size: 0.65rem; color: var(--mid); margin-top: 8px; line-height: 1.5;
}}
.detail-funds {{
  font-size: 0.63rem; color: var(--mid); margin-top: 8px;
}}
.detail-funds-list {{ margin-top: 3px; }}
.fund-tag {{
  display: inline-block; padding: 2px 6px; margin: 2px 2px 0 0;
  background: var(--faint); font-size: 0.6rem;
}}

/* ── Labor bar inside detail ── */
.labor-bar-wrap {{ margin-top: 10px; }}
.labor-bar-track {{ height: 18px; background: var(--faint); position: relative; border-radius: 2px; overflow: hidden; }}
...
.radio-group {{ display: flex; flex-direction: column; gap: 4px; padding: 2px 0; }}
.radio-item  {{
  display: flex; align-items: center; gap: 7px;
  font-size: 0.72rem; cursor: pointer; padding: 3px 0;
}}
.radio-item input {{ accent-color: var(--ink); cursor: pointer; margin: 0; }}

/* ── Service legend in sidebar ── */
.svc-legend {{ display: flex; flex-direction: column; gap: 5px; padding: 4px 0; }}
.svc-legend-item {{
  display: flex; align-items: center; gap: 7px;
  font-size: 0.68rem; cursor: pointer; padding: 2px 0;
}}
.svc-legend-item:hover {{ text-decoration: underline; }}
...
@media (max-width: 900px) {{
  ...
  .explorer-col   {{ height: 40vh; border-right: none; border-bottom: var(--border); }}
...
<div class="main">
  ...
  <div class="detail-card" id="detailCard">
    <div class="detail-head">
      <span class="detail-prog-name" id="dProgName"></span>
      <span class="detail-breadcrumb" id="dBreadcrumb"></span>
      <button class="detail-close" id="detailClose">&#x2715;</button>
    </div>
    <div class="detail-body">
      <div class="detail-section">
        <div class="detail-meta-grid" id="dMetaGrid"></div>
        <div class="labor-bar-wrap" id="dLaborBar"></div>
        <div class="detail-bsl" id="dBsl"></div>
        <div class="detail-funds" id="dFunds"></div>
      </div>
      <div class="detail-section">
        <div class="detail-section-label">Program description</div>
        <div class="detail-desc" id="dDesc"></div>
        <div class="detail-bsl" id="dBslDesc" style="margin-top:12px"></div>
      </div>
    </div>
  </div>
...
function showDetail(prog) {{
  ...
  $('dProgName').textContent   = prog.name;
  $('dBreadcrumb').textContent = `${{prog._dept}}  ·  ${{prog._svc}}`;

  // Meta grid
  const ftes = prog.ftes != null ? prog.ftes.toFixed(1) + ' FTEs' : '— FTEs';
  $('dMetaGrid').innerHTML = `
  ...
  $('dLaborBar').innerHTML = `
    <div class="detail-meta-item" style="margin-bottom:8px">
      <div class="detail-meta-val">${{ftes}}</div>
      <div class="detail-meta-key">Full-time equivalents</div>
    </div>
    <div class="detail-section-label">Labor vs non-labor</div>
  ...
  // BSL
  $('dBsl').innerHTML = prog.bsl
    ? `<strong>Budget Summary Level:</strong> ${{prog.bsl}}`
    : '';

  // Funds
  if (prog.funds && prog.funds.length) {{
    $('dFunds').innerHTML = `<div class="detail-section-label" style="margin-top:10px">Funds</div>
      <div class="detail-funds-list">${{prog.funds.map(f => `<span class="fund-tag">${{f}}</span>`).join('')}}</div>`;
  }} else {{
    $('dFunds').innerHTML = '';
  }}
  ...
  if (prog.description) {{
    ...
    descEl.textContent = 'No description available in the 2026 Adopted Budget Book for this program.';
    descEl.classList.add('empty');
  }}

  // BSL description
  $('dBslDesc').innerHTML = prog.bslDesc
    ? `<strong>BSL context:</strong> ${{prog.bslDesc}}`
    : '';

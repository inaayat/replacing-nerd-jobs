/**
 * Sticky Notes data layer — dependency-free ESM.
 * Browser board uses localStorage; the Chrome extension mirrors the same
 * shape in chrome.storage.local under STORAGE_KEY.
 */
...
export function upsertNote(store, note) {
  const normalized = normalizeNote(note);
  if (!normalized) return normalizeStore(store);
  const next = normalizeStore(store);
  const idx = next.notes.findIndex((n) => n.id === normalized.id);
  normalized.updatedAt = nowIso();
  if (idx === -1) next.notes.push(normalized);
  else next.notes[idx] = { ...next.notes[idx], ...normalized };
  return next;
}

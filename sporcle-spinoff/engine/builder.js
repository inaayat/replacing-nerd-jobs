...
let quiz = { title: '', blurb: '', timeLimitSec: undefined, shuffle: false, type: 'text-entry', items: [] };
    ...
    <div>
      ...
      <div class="b-step">
        ...
        <div class="b-field-row">
          ...
          <div class="b-field b-toggle"><label><input id="f-shuffle" type="checkbox"> Shuffle question order</label></div>
        </div>
      ...
      <div class="b-step">
        <div class="b-step-head"><span class="b-step-num">4</span><span class="b-step-title">Publish</span></div>
...
function updatePreview() {
  ...
  document.getElementById('id-preview').innerHTML =
    `Will appear at <code>/sporcle-spinoff/play.html?quiz=${slugify(idInput.value)}</code>, filed under <b>${TEMPLATES.find((t) => t.id === quiz.type).label}</b> on the catalog page.`;

  const hint = document.getElementById('validation-hint');
  const btn = document.getElementById('publish-btn');
  if (!quiz.title.trim()) {
    hint.textContent = 'Add a title before publishing.'; hint.style.display = 'block'; btn.disabled = true;
  } else if (count < 2) {
    hint.textContent = 'Add at least 2 questions before publishing.'; hint.style.display = 'block'; btn.disabled = true;
  } else {
...
async function publish() {
  ...
  try {
    const res = await fetch('/api/save-quiz', {
      ...
      body: JSON.stringify({ quiz: currentQuizObject(), mode: 'publish' }),
    });
    const data = await res.json();
    if (!res.ok) throw new Error(data.error || `Request failed (${res.status})`);
    toast.textContent = `Published — now live at ${data.url}`;
    toast.className = 'b-toast ok';
  } catch (err) {
    toast.textContent = `Couldn't publish: ${err.message}`;

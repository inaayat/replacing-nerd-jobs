...
as the rest of the site; account delete stays on `/account.html`). Trip-local **Outfits** (named looks, never cubes; optional date and dress
code) are on by default, including as groups on List and By cube. After the trip you can mark what you actually
...
cross-street columns, so there is no key to join on. `im-filmin-here/streets.js`
normalizes street names on both sides with one function and resolves them against
a committed centerline + intersection index (`im-filmin-here/data/streets.json`,
generated by `node scripts/pull-im-filmin-here-streets.mjs`). 97.5% of segment
mentions place; the rest are reported in the UI rather than dropped. Notes in
[`docs/filming-locations-plan.md`](docs/filming-locations-plan.md).
...
user (`wedding_boards`) via `api/wedding.js` (`/api/wd-board`). Signing in is
...
button adds one compact visual preview below the body (click the preview to
type under it): direct image/video URLs, Pinterest images,
Instagram posts/Reels, TikToks, and YouTube. Instagram stays a still — its
official embed is a mini profile page and does not fit a card. YouTube and
TikTok stay behind a play control so the card keeps its sticky-note shape; the media
...
the whole map: tap to edit, one-finger pan, two fingers to zoom and drag the board
at once (lift one and the board keeps following the other), long-press to
multi-select, and **pinch a note to resize it** — both fingers on the same card
scale it, so there is no small handle to find. The 44px corner handle is still
there on the selected or open note.
...
compacts the site nav when it runs standalone. Eligible browsers get an
**Install app** control; the scoped service worker caches the static shell
(never APIs; JS/CSS are network-first so a deploy lands on the next open), so the local-first board opens offline and queued account changes
sync when the connection returns. The installed app is an OS share target:
shared text/URLs become a note, and its app shortcut opens a fresh note.
...
node scripts/test-sticky-notes-pwa.mjs

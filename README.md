...
Each ED also carries Community District, City Council, Assembly, State Senate,
and Congressional ids so later layers can join on `ElectDist`. Boundaries come
from DCP’s ArcGIS services (the datasets behind
[NYPL’s political-districts guide](https://libguides.nypl.org/nycboundaries/political)).
Catalog is `world-in-nyc/data/enclaves.json`; geometries are generated by
`node scripts/build-world-in-nyc.mjs` and committed. No new Vercel function.
